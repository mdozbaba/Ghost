        ...
        it('import multiple users, tags, posts, clients', function () {
            ...
            return dataImporter.doImport(exportData, clonedImportOptions)
                .then(function () {
                    ...
                    clients.models[0].get('slug').should.eql('ghost-admin');
                    clients.models[1].get('slug').should.eql('ghost-scheduler');
                    clients.models[2].get('slug').should.eql('ghost-auth');
                    clients.models[3].get('slug').should.eql('ghost-backup');
                    clients.models[4].get('slug').should.eql('ghost-frontend');
                    clients.models[4].get('secret').should.eql('678910');
                    clients.models[5].get('slug').should.eql('ghost-new');
                    clients.models[5].get('secret').should.eql('88888');

                    trustedDomains.models.length.should.eql(3);
                    trustedDomains.models[0].get('trusted_domain').should.eql('https://test.com');
                    trustedDomains.models[0].get('client_id').should.eql(testUtils.DataGenerator.forKnex.clients[0].id);

                    trustedDomains.models[1].get('trusted_domain').should.eql('https://example.com');
                    trustedDomains.models[1].get('client_id').should.eql(testUtils.DataGenerator.forKnex.clients[0].id);

                    trustedDomains.models[2].get('trusted_domain').should.eql('https://world.com');
                    trustedDomains.models[2].get('client_id').should.eql(clients.models[5].id);
                });
        });
    });
});
